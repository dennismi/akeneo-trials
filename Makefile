...
assets-dev:
	$(CMD_ON_PROJECT) rm -rf public/bundles public/js
	$(PHP_RUN) bin/console pim:installer:assets --symlink --clean --env=dev
...
.PHONY: front-dev
front-dev: cache-dev assets-dev css javascript-dev 

.PHONY: front
front: cache assets css javascript-prod 
...
.PHONY: cache-dev
cache-dev:
	$(CMD_ON_PROJECT) rm -rf var/cache && $(PHP_RUN) bin/console cache:warmup --env=dev
...
composer.lock: composer.json
	$(PHP_RUN) -d memory_limit=4G /usr/bin/composer update

vendor: composer.lock
	$(PHP_RUN) -d memory_limit=4G /usr/bin/composer install
